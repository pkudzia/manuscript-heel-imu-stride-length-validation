%% PeerJ - Sports Medicine and Rehabilitation submission.
%% Uses the wlpeerj document class (Overleaf/PeerJ official template).
%% Remove the `lineno' option for camera-ready submission.

\documentclass[fleqn,10pt,lineno]{wlpeerj}

%% Math (amsmath/amsfonts/amssymb already loaded by wlpeerj)
\usepackage{bm}

%% Tables and figures (booktabs and graphicx already loaded by wlpeerj)
\usepackage{array}
\usepackage{tabularx}
...
%% Separate paragraphs visibly in the review copy without changing the PeerJ class.
...
\corrauthor[1]{Pawel Kudzia}{pawel.kudzia5@gmail.com}
...
\textbf{Gait metrics such as stride length are sensitive markers of mobility decline, fall risk, and cognitive deterioration in older adults.}
Greater stride-to-stride variability predicts incident falls in older adults \citep{Maki1997, Verghese2009}. Slower gait also predicts falls and tracks cognitive decline \citep{Verghese2009, Montero-Odasso2012}. A meta-analysis reported that a baseline stride-length threshold of 0.64~m or less predicted adverse clinical events and mortality in older adults \citep{Bytyci2021}. Instrumented walkways and motion capture measure stride length accurately but restrict assessment to short walks in laboratory settings. This leaves daily-living and clinical contexts without these tools. Wearable inertial measurement units (IMUs) can estimate stride length outside the laboratory, but the quality of these estimates depends on both the measurement setup and the processing method.

\textbf{Sensor quality, secure attachment, placement, and algorithm design all affect stride-length accuracy with foot-mounted IMUs.}
The most common approach estimates stride length by integrating acceleration twice. Noise and small sensor errors accumulate during this process and cause the estimated position to drift. A sensor that moves or rotates relative to the shoe can introduce further error because it no longer measures the foot from a consistent position and orientation. The hardware must therefore provide low-noise signals, and the sensor must remain firmly attached \citep{Wahlstrom2020, Kuderle2022}. Zero-velocity updates (ZUPTs) limit drift by resetting the integration during mid-stance, when the foot is flat and motionless \citep{Foxlin2005, Wahlstrom2020}. Detecting this period correctly is important because it marks where each stride begins and ends. Adaptive methods that account for each person's walking pattern remain more reliable across walking speeds than a single fixed threshold \citep{Li2023}.

\textbf{Fixed amplitude thresholds can miss genuine stance phases and misclassify other stationary periods.}
During mid-stance the foot is still. Both the sensor's rotational speed (the gyroscope norm, the combined magnitude of triaxial angular velocity) and the moment-to-moment variation in its acceleration then drop close to zero. A threshold detector labels a sample as stance whenever both signals fall below fixed cutoffs \citep{Skog2010}. This works for steady walking, but during slow or irregular walking, such as the shuffling gait of Parkinson's disease, a stance phase may never become quiet enough to cross the cutoff \citep{Foxlin2005}. The detector then misses the stance and merges two strides into one. During pauses such as standing or turning, the foot is also still, so the detector fires a false stance that shortens the surrounding strides.

\textbf{Bilateral ZUPT checks each candidate stance against movement of the opposite foot.}
During normal walking, one foot lifts from the ground shortly after the opposite foot enters stance. The detector uses this relationship to confirm that a low-motion period represents walking rather than quiet standing, turning, or stopping. In technical terms, it accepts an ipsilateral stance candidate when a toe-off occurs on the contralateral foot within the expected time window \citep{Arens2021}. A recovery step also searches for quiet periods near unmatched opposite-foot toe-offs, allowing the detector to recover stances that a threshold alone missed. This check is particularly relevant to the slow and variable walking of older adults and people with cognitive impairment.
...
Foot- and shank-mounted IMU validation studies typically examine single-task walking at a steady pace in healthy adults \citep{Sijobert2015}. Cognitive dual-task walking slows gait and increases stride-to-stride variability \citep{Beauchet2005, Yogev-Seligmann2008, Hausdorff2008}. Slower, more variable strides widen the range of speeds a pipeline must resolve and make mid-stance phases harder to detect. Older adults, especially those with cognitive impairment, walk more slowly and with greater variability. An IMU pipeline must therefore be validated under these conditions, not solely on steady single-task walking. One heel-mounted IMU study included a single dual-task condition and assessed intra-session reliability in 101 middle-aged adults \citep{Boutaayamou2025}. That study did not validate against a criterion reference, span graded cognitive load, or include older adults. Prior validation in pathological cohorts against instrumented treadmills used fixed speeds rather than variable gait speeds \citep{Laidig2021}. Dual-task gait assessment carries clinical relevance because it supports fall-risk screening and cognitive monitoring \citep{Montero-Odasso2012, Commandeur2018, Springer2006}. To our knowledge, no prior study has validated gait-event detection and stride-length estimation against a criterion walkway in older adults under graded cognitive load across MoCA-defined cognitive status.

\textbf{The second gap concerns heel-mounted accuracy in older adults.}
The only large heel-specific study examined single-stride variability at multiple shoe-collar positions, including the heel, in healthy adults (mean age 25 years) \citep{Kuderle2022}. A prior study demonstrated bilateral validation in post-stroke gait ($n = 8$, wearable-mounted sensors) \citep{Arens2021}. A heel-mounted bilateral ZUPT stance-detection and stride-length pipeline has not been validated against a criterion walkway in older adults walking under graded cognitive load.
...
(a) Filtered sagittal-plane signals from the left (black) and right (blue) heel IMUs during one out-and-back trial. Labels mark activity phases, red bars mark off-mat intervals, and the box is expanded in panel~c.
(b) Heel IMUs clipped to the posterolateral shoe using 3D-printed housings during GaitRite walking.
(c) Mean gyroscope signal over a heel-strike-defined gait cycle, pooled across participants and conditions. Shading shows $\pm 1$~SD, dashed lines mark heel strike and toe off, and blue marks swing.}
\label{fig:methods}
\end{figure*}
...
We will deposit the de-identified stride-level data and the analysis code that support the findings of this study in a public repository (Zenodo) and release them under an open licence upon publication. We will additionally host the analysis code on GitHub, and we will provide a persistent DOI and the repository URL at that time.
